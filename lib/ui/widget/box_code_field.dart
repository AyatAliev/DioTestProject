...
class _PinCodeFieldsState extends State<PinCodeFields> {
  ...
  void initState() {
    ...
    _inputList = List.filled(widget.length, "0", growable: false);
    _initializeValues();
    super.initState();
  }
  ...
  void _setTextToInput(String data) async {
    List<String> replaceInputList = List.filled(widget.length, "null", growable: false);
  ...
  Color _getColorFromIndex(int index) {
    ...
    if (((_selectedIndex == index) ||
        ...
        _focusNode.hasFocus) {
      return widget.activeBorderColor ?? Colors.white;
  ...
  Color _getBackgroundColorFromIndex(int index) {
    if (!widget.enabled) {
      return widget.fieldBackgroundColor ?? Colors.white;
    }
    if (((_selectedIndex == index) ||
        (_selectedIndex == index + 1 && index + 1 == widget.length)) &&
        _focusNode.hasFocus) {
      return widget.activeBackgroundColor ?? Colors.white;
    } else if (_selectedIndex > index) {
      return widget.fieldBackgroundColor ?? Colors.white;
    }
    return widget.fieldBackgroundColor ?? Colors.white;
